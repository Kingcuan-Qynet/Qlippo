...
### 4. Email verification / authentication

Confirmation links use `/api/v1/auth/callback?next=%2Fonboarding`. The callback
exchanges Supabase's PKCE `code` for a server-side session before redirecting.
This prevents the common failure where verification sends the browser directly
to `/feed` without an SSR session cookie and the protected layout sends the user
back to `/login`.

In Supabase Dashboard → Authentication → URL Configuration, add the exact
production callback URL, for example `https://your-domain.com/api/v1/auth/callback`.
For production, `NEXT_PUBLIC_SITE_URL` must be the canonical HTTPS site URL with
no trailing slash. Never expose `SUPABASE_SERVICE_ROLE_KEY` to client-side code.
